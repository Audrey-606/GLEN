...
class Trainer(DefaultTrainer):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        for name, param in model.named_parameters():
            if "length_module" in name:
                param.requires_grad = True

        # 解冻Backbone最后两层
        # name, param in model.named_parameters():
            #if "backbone.layer4" in name:  # ResNet的第四个stage
            #        param.requires_grad = True
            #if "backbone.layer3" in name:  # 解冻更底层特征
            #        param.requires_grad = True

        # 解冻FPN
        # for name, param in model.named_parameters():
            # if "fpn" in name.lower():  # 包含FPN相关参数
                # param.requires_grad = True

        # 解冻检测头
        #for name, param in model.named_parameters():
            #if "roi_heads.box_head" in name:  # Faster R-CNN的检测头
                #param.requires_grad = True
            #if "roi_heads.mask_head" in name:  # Mask R-CNN的分割头
                #param.requires_grad = True
            #if "proposal_generator" in name:  # RPN网络
                #param.requires_grad = True
    ...
    def train_loop(self, start_iter: int, max_iter: int):
        ...
        with EventStorage(start_iter) as self.storage:
            ...
            for self.iter in range(start_iter, max_iter):
                ...
                self.run_step()
                # ============= 梯度 =============
                if self.iter % 50 == 0:  # 每50次迭代检查一次
                    grads = []
                    for name, param in self.model.named_parameters():
                        if param.grad is not None and "length_module" in name:
                            grads.append(param.grad.abs().mean().item())
                    if grads:  # 避免空列表
                        avg_grad = sum(grads) / len(grads)
                        logger.info(
                            f"Iter {self.iter}: 长度模块梯度均值={avg_grad:.6f} | "
                            f"当前loss_length={self.storage.latest().get('loss_length', 'N/A')}"
                        )
                        self.storage.put_scalar("avg_grad_length", avg_grad)
                # 添加梯度裁剪
