...
class VisualizationDemo(object):
    ...
    def run_on_image(self, image):
        ...
        # Handle text visualization or regular visualization
        if self.vis_text:
            visualizer = TextVisualizer(image, self.metadata, instance_mode=self.instance_mode, cfg=self.cfg)
        else:
            visualizer = Visualizer(image, self.metadata, instance_mode=self.instance_mode)

        # Check and extract 'lengths' if available


            for result in predictions.get("instances", []):
                result["predicted_length"] = lengths  # Attach the predicted length to each instance if required

        # Handle visualization of different outputs
