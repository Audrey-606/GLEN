...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        # Extract pixel_to_mm_ratio and pixel_length from annotations
        pixel_to_mm_ratios = []
        pixel_lengths = []

        if "annotations" in dataset_dict:
            for anno in dataset_dict["annotations"]:
                # 提取参数并存储，假设每个注释都有相应字段
                # print(anno)
                pixel_to_mm_ratios.append(anno.get("pixel_to_mm_ratio", None))
                pixel_lengths.append(anno.get("pixel_length", None))
        # 图像读取及增强部分
        try:
        ...
        # Remove if you don't do semantic/panoptic segmentation.
        ...
        # Handle pre-computed proposals if needed
        ...
        if "annotations" in dataset_dict:
            # Remove unnecessary fields and add pixel_to_mm_ratio and pixel_length
            for anno in dataset_dict["annotations"]:
                ...
                # 将提取的参数添加到实例中
                anno["pixel_to_mm_ratio"] = pixel_to_mm_ratios.pop(0)  # 逐个取出
                anno["pixel_length"] = pixel_lengths.pop(0)
            ...
            if len(annos) > 0:
                # 提取所有pixel_length值，确保类型和设备一致
                pixel_lengths = [anno["pixel_length"] for anno in annos]
                # 转换为Tensor（假设pixel_length是浮点数）
                pixel_lengths_tensor = torch.tensor(
                    pixel_lengths,
                    dtype=torch.float32,
                    device=instances.gt_boxes.device  # 保持与现有数据相同的设备
                )
                # 添加到Instances对象中，字段名如"gt_pixel_length"
                instances.gt_pixel_length = pixel_lengths_tensor
            # print(instances)

            # Recompute box if needed
