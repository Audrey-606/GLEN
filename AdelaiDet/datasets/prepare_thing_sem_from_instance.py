...
def _process_instance_to_semantic(anns, output_semantic, img, categories):
    ...
    for ann in anns:
        ...
        if category_id not in categories:
            print(f"Warning: category_id {category_id} not found in categories")
        mask = annToMask(ann, img_size)
        output[mask == 1] = categories.get(category_id, 0) + 1  # 这里加了一个默认值0
...
if __name__ == "__main__":
    ...
    if args.dataset_name == "coco_seedling":
